...
class CancelPremiumViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.template_name_redirect = 'word_bank/premium.html'
...
class PasswordResetViewTestCase(TestCase):
    ...
    def test_password_reset_view_email_doesnt_exist_POST(self):
        form_data = {
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, self.template_name)
        self.assertTemplateNotUsed(self.template_name_redirect)
        self.assertFormError(response, 'form', 'email', GUI_MESSAGES['forms']['error_email_doesnt_exist'])
...
class PasswordResetCheckViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        token = cls.token_generator.make_token(cls.test_user)
        ...
        cls.invalid_token = CustomUserToken.objects.create(
            ...
            token_type=token_type,
            token=generate_user_token(cls.test_user.id),
        )
...
class SetPasswordViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.test_user = cls.User.objects.get(username='test_user')
        token_type = CustomUserTokenType.objects.get(name='Password reset')
        token = cls.token_generator.make_token(cls.test_user)
    ...
    def test_set_password_view_form_valid_POST(self):
        ...
        login_password_old = self.client.login(username='test_user', password='test_password')
        self.assertFalse(login_password_old)
        self.client.logout()
        login_password_new = self.client.login(username='test_user', password='new_password')
